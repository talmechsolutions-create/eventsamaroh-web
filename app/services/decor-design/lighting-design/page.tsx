import Image from "next/image";
import Link from "next/link";
import { CheckCircle } from "lucide-react";

export const metadata = {
  title: "Event Lighting Design in Pune | Wedding & Corporate Lighting Experts",
  description:
    "Professional event lighting design services in Pune including wedding ambient lighting, stage illumination, architectural lighting and premium event lighting setups by EventSamaroh.",
};

export default function LightingDesignPage() {
  return (
    <main className="overflow-hidden">

      {/* ================= HERO ================= */}
      <section className="relative min-h-[75vh] sm:min-h-[85vh] flex items-center justify-center text-center text-white">

        <Image
          src="/images/services/decor-design/hero.webp"
          alt="Event Lighting Design in Pune"
          fill
          priority
          sizes="100vw"
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black/80" />

        <div className="relative z-10 px-6 max-w-4xl">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-heading font-semibold leading-tight">
            Event Lighting Design in Pune
          </h1>

          <p className="mt-6 text-lg sm:text-xl text-white/85 max-w-2xl mx-auto">
            Transform venues with premium ambient lighting,
            stage illumination and customized lighting concepts.
          </p>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Plan Lighting Consultation
          </Link>
        </div>
      </section>

      {/* ================= OVERVIEW ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6 text-center">

          <h2 className="text-3xl sm:text-4xl font-heading text-brand-primary">
            Professional Event Lighting Solutions
          </h2>

          <p className="mt-8 text-brand-muted leading-relaxed">
            EventSamaroh offers premium lighting design services in Pune
            for weddings, receptions, corporate events and private celebrations.
          </p>

          <p className="mt-6 text-brand-muted leading-relaxed">
            From warm ambient lighting for romantic weddings to
            dynamic stage lighting for corporate events,
            we create immersive visual experiences.
          </p>

        </div>
      </section>

      {/* ================= WHAT WE OFFER ================= */}
      <section className="py-20 bg-brand-soft">
        <div className="max-w-6xl mx-auto px-6 text-center">

          <h2 className="text-3xl font-heading text-brand-primary">
            Lighting Services Include
          </h2>

          <div className="grid md:grid-cols-3 gap-10 mt-16">

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Ambient Wedding Lighting
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Warm and romantic lighting concepts for wedding venues.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Stage & Spotlight Illumination
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Professional lighting for stages and performances.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Architectural & Venue Lighting
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Highlight venue structures and decorative elements.
              </p>
            </div>

          </div>
        </div>
      </section>

      {/* ================= FAQ ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6">

          <h2 className="text-3xl font-heading text-brand-primary text-center">
            Event Lighting FAQs
          </h2>

          <div className="mt-12 space-y-8">

            <div>
              <h3 className="font-semibold text-lg">
                Do you provide customized lighting themes?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, lighting concepts are customized to match
                your decor and event style.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Can lighting enhance outdoor venues?
              </h3>
              <p className="mt-2 text-brand-muted">
                Absolutely. We provide weather-safe lighting
                setups for outdoor celebrations.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Do you coordinate with stage and decor teams?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, our lighting designers collaborate
                closely with decor and AV teams.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                When should lighting be finalized?
              </h3>
              <p className="mt-2 text-brand-muted">
                We recommend finalizing lighting design
                3–5 weeks before the event date.
              </p>
            </div>

          </div>

        </div>
      </section>

      {/* ================= RELATED SERVICES ================= */}
      <section className="py-16 bg-brand-soft">
        <div className="max-w-5xl mx-auto px-6 text-center">

          <h2 className="text-2xl font-heading text-brand-primary">
            Explore More Decor Services
          </h2>

          <div className="mt-8 flex flex-wrap justify-center gap-4 text-brand-accent">

            <Link href="/services/decor-design/stage-mandap" className="hover:underline">
              Stage & Mandap
            </Link>

            <Link href="/services/decor-design/floral-styling" className="hover:underline">
              Floral Styling
            </Link>

            <Link href="/services/decor-design/theme-concepts" className="hover:underline">
              Theme Concepts
            </Link>

          </div>

        </div>
      </section>

      {/* ================= CTA ================= */}
      <section className="relative py-24 overflow-hidden">

        <Image
          src="/images/services/decor-design/hero.webp"
          alt="Event Lighting Design Consultation in Pune"
          fill
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/70 to-black/90" />

        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center text-white">

          <h2 className="text-3xl sm:text-4xl font-heading font-semibold">
            Illuminate Your Celebration with Premium Lighting
          </h2>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Book Lighting Consultation
          </Link>

        </div>
      </section>

    </main>
  );
}