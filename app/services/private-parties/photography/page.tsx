import Image from "next/image";
import Link from "next/link";
import { CheckCircle } from "lucide-react";

export const metadata = {
  title: "Private Party Photography in Pune | Professional Event Photographers",
  description:
    "Professional private party photography services in Pune including birthday photography, anniversary shoots, candid moments and premium event coverage by EventSamaroh.",
};

export default function PrivatePhotographyPage() {
  return (
    <main className="overflow-hidden">

      {/* ================= HERO ================= */}
      <section className="relative min-h-[75vh] sm:min-h-[85vh] flex items-center justify-center text-center text-white">

        <Image
          src="/images/services/private-parties/hero.webp"
          alt="Private Party Photography in Pune"
          fill
          priority
          sizes="100vw"
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black/80" />

        <div className="relative z-10 px-6 max-w-4xl">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-heading font-semibold leading-tight">
            Private Party Photography in Pune
          </h1>

          <p className="mt-6 text-lg sm:text-xl text-white/85 max-w-2xl mx-auto">
            Capture unforgettable memories with professional photography
            coverage for private celebrations and social gatherings.
          </p>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Book Photography Services
          </Link>
        </div>
      </section>

      {/* ================= OVERVIEW ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6 text-center">

          <h2 className="text-3xl sm:text-4xl font-heading text-brand-primary">
            Professional Event Photography for Private Parties
          </h2>

          <p className="mt-8 text-brand-muted leading-relaxed">
            EventSamaroh provides premium private party photography services
            in Pune for anniversaries, milestone birthdays, cocktail evenings
            and exclusive celebrations.
          </p>

          <p className="mt-6 text-brand-muted leading-relaxed">
            Our experienced photographers specialize in candid photography,
            group portraits, detail shots and cinematic coverage to ensure
            every special moment is beautifully preserved.
          </p>

        </div>
      </section>

      {/* ================= WHAT WE OFFER ================= */}
      <section className="py-20 bg-brand-soft">
        <div className="max-w-6xl mx-auto px-6 text-center">

          <h2 className="text-3xl font-heading text-brand-primary">
            Photography Services Include
          </h2>

          <div className="grid md:grid-cols-3 gap-10 mt-16">

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Candid Photography
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Natural and spontaneous moments captured professionally.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Portrait & Group Shots
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Styled portraits and family group photography.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Event Highlight Coverage
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Coverage of performances, speeches and celebration highlights.
              </p>
            </div>

          </div>
        </div>
      </section>

      {/* ================= FAQ ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6">

          <h2 className="text-3xl font-heading text-brand-primary text-center">
            Private Party Photography FAQs
          </h2>

          <div className="mt-12 space-y-8">

            <div>
              <h3 className="font-semibold text-lg">
                Do you provide candid photography?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, our photographers specialize in candid
                and documentary-style event coverage.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                How many photographers will cover the event?
              </h3>
              <p className="mt-2 text-brand-muted">
                Team size depends on event scale and guest count,
                ensuring complete coverage.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Do you provide edited digital photos?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, professionally edited high-resolution images
                are delivered post-event.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Can photography be combined with videography?
              </h3>
              <p className="mt-2 text-brand-muted">
                Absolutely. We offer combined photography
                and videography packages.
              </p>
            </div>

          </div>

        </div>
      </section>

      {/* ================= RELATED SERVICES ================= */}
      <section className="py-16 bg-brand-soft">
        <div className="max-w-5xl mx-auto px-6 text-center">

          <h2 className="text-2xl font-heading text-brand-primary">
            Explore More Private Party Services
          </h2>

          <div className="mt-8 flex flex-wrap justify-center gap-4 text-brand-accent">

            <Link href="/services/private-parties/theme-decor" className="hover:underline">
              Theme & Decor
            </Link>

            <Link href="/services/private-parties/catering-entertainment" className="hover:underline">
              Catering & Entertainment
            </Link>

            <Link href="/services/private-parties/venue-booking" className="hover:underline">
              Venue Booking
            </Link>

          </div>

        </div>
      </section>

      {/* ================= CTA ================= */}
      <section className="relative py-24 overflow-hidden">

        <Image
          src="/images/services/private-parties/hero.webp"
          alt="Private Party Photography Services in Pune"
          fill
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/70 to-black/90" />

        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center text-white">

          <h2 className="text-3xl sm:text-4xl font-heading font-semibold">
            Preserve Every Special Moment
          </h2>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Book Photography Consultation
          </Link>

        </div>
      </section>

    </main>
  );
}