import Image from "next/image";
import Link from "next/link";
import { CheckCircle } from "lucide-react";

export const metadata = {
  title: "Annual Corporate Meeting Management in Pune | AGM Event Planners",
  description:
    "Professional annual meeting and AGM event management services in Pune including venue coordination, AV setup, delegate management and complete execution by EventSamaroh.",
};

export default function AnnualMeetingsPage() {
  return (
    <main className="overflow-hidden">

      {/* ================= HERO ================= */}
      <section className="relative min-h-[75vh] sm:min-h-[85vh] flex items-center justify-center text-center text-white">

        <Image
          src="/images/services/corporate/hero.webp"
          alt="Annual Corporate Meeting Management in Pune"
          fill
          priority
          sizes="100vw"
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/60 to-black/80" />

        <div className="relative z-10 px-6 max-w-4xl">
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-heading font-semibold leading-tight">
            Annual Corporate Meeting Management in Pune
          </h1>

          <p className="mt-6 text-lg sm:text-xl text-white/85 max-w-2xl mx-auto">
            Structured and professionally executed annual meetings,
            board meetings and corporate AGMs.
          </p>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Plan Annual Meeting
          </Link>
        </div>
      </section>

      {/* ================= OVERVIEW ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6 text-center">

          <h2 className="text-3xl sm:text-4xl font-heading text-brand-primary">
            Professional AGM & Corporate Meeting Organizers in Pune
          </h2>

          <p className="mt-8 text-brand-muted leading-relaxed">
            EventSamaroh provides complete annual meeting and AGM event
            management services in Pune for corporations,
            organizations and enterprises.
          </p>

          <p className="mt-6 text-brand-muted leading-relaxed">
            We ensure smooth execution of shareholder meetings,
            leadership summits, strategy sessions and internal
            corporate gatherings with technical precision.
          </p>

        </div>
      </section>

      {/* ================= WHAT WE OFFER ================= */}
      <section className="py-20 bg-brand-soft">
        <div className="max-w-6xl mx-auto px-6 text-center">

          <h2 className="text-3xl font-heading text-brand-primary">
            Annual Meeting Services We Provide
          </h2>

          <div className="grid md:grid-cols-3 gap-10 mt-16">

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Venue & Seating Arrangement
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Conference halls and structured seating layouts.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                AV & Presentation Support
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Audio systems, projection setup and technical supervision.
              </p>
            </div>

            <div className="bg-white p-8 rounded-2xl shadow-md">
              <CheckCircle className="w-10 h-10 text-brand-accent mx-auto" />
              <h3 className="mt-6 font-semibold text-brand-primary">
                Registration & Guest Handling
              </h3>
              <p className="mt-4 text-sm text-brand-muted">
                Delegate check-in and professional guest assistance.
              </p>
            </div>

          </div>
        </div>
      </section>

      {/* ================= FAQ ================= */}
      <section className="py-20 bg-white">
        <div className="max-w-4xl mx-auto px-6">

          <h2 className="text-3xl font-heading text-brand-primary text-center">
            Annual Meeting FAQs
          </h2>

          <div className="mt-12 space-y-8">

            <div>
              <h3 className="font-semibold text-lg">
                Do you manage large shareholder meetings?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, we manage both small board meetings
                and large-scale shareholder AGMs.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Can you handle technical presentations?
              </h3>
              <p className="mt-2 text-brand-muted">
                Our AV team ensures smooth presentations
                and uninterrupted technical support.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Do you coordinate hospitality services?
              </h3>
              <p className="mt-2 text-brand-muted">
                Yes, we manage catering, refreshments
                and corporate hospitality arrangements.
              </p>
            </div>

            <div>
              <h3 className="font-semibold text-lg">
                Is on-site supervision included?
              </h3>
              <p className="mt-2 text-brand-muted">
                Our event managers remain present
                throughout the meeting for smooth execution.
              </p>
            </div>

          </div>

        </div>
      </section>

      {/* ================= RELATED SERVICES ================= */}
      <section className="py-16 bg-brand-soft">
        <div className="max-w-5xl mx-auto px-6 text-center">

          <h2 className="text-2xl font-heading text-brand-primary">
            Explore More Corporate Services
          </h2>

          <div className="mt-8 flex flex-wrap justify-center gap-4 text-brand-accent">

            <Link href="/services/corporate-events/conferences" className="hover:underline">
              Corporate Conferences
            </Link>

            <Link href="/services/corporate-events/product-launch" className="hover:underline">
              Product Launch Events
            </Link>

            <Link href="/services/corporate-events/stage-av" className="hover:underline">
              Stage & AV Setup
            </Link>

          </div>

        </div>
      </section>

      {/* ================= CTA ================= */}
      <section className="relative py-24 overflow-hidden">

        <Image
          src="/images/services/corporate/hero.webp"
          alt="Annual Corporate Meeting Planning in Pune"
          fill
          className="object-cover"
        />

        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/70 to-black/90" />

        <div className="relative z-10 max-w-3xl mx-auto px-6 text-center text-white">

          <h2 className="text-3xl sm:text-4xl font-heading font-semibold">
            Organize Your Annual Meeting with Confidence
          </h2>

          <Link
            href="/contact"
            className="inline-block mt-10 px-8 py-3 bg-brand-accent text-white rounded-full hover:scale-105 transition"
          >
            Schedule Consultation
          </Link>

        </div>
      </section>

    </main>
  );
}